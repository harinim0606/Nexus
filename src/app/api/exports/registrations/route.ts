...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'pdf') {
      ...
      let y = 16;
      doc.setFontSize(14);
      doc.text(`Registrations — ${event.name}`, 14, y);
      y += 10;
      doc.setFontSize(10);
      regs.forEach((r, i) => {
        if (y > 280) {
        ...
        const line = `${i + 1}. ${r.user.name}  |  ${r.user.email}  |  ${r.status}`;
        doc.text(line, 14, y);
        y += 7;
      });
      ...
      return new NextResponse(buf, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="nexus-registrations-${eventId}.pdf"`,
        },
      });
    }
    ...
    return new NextResponse(Buffer.from(ab), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="nexus-registrations-${eventId}.xlsx"`,
      },
    });
  } catch {
